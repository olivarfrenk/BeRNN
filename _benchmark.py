...
class Trial(object):
    ...
    def add_x_noise(self):
        """Add input noise."""
        self.x += self.config['rng'].standard_normal(self.x.shape) * self._sigma_x
...
def delaygo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim_locs = rng.random(batch_size) * 2 * np.pi
        # stim_ons  = int(500/dt)
        stim_ons = int(rng.uniform(300, 700) / dt)  # int(rng.choice([300, 500, 700])/dt)
        stim_offs = stim_ons + int(rng.uniform(200, 600) / dt)  # int(rng.choice([200, 400, 600])/dt)
        fix_offs = stim_offs + int(rng.uniform(200, 1600) / dt)  # int(rng.choice([200, 400, 800, 1600])/dt)
...
def _contextdm(config, mode, attend_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        # Time of stimuluss on/off
        stim_on = int(rng.uniform(300,700)/dt)
        # stim_ons = (np.ones(batch_size)*stim_on).astype(int)
        stim_ons = int(stim_on)
        stim_dur = int((rng.uniform(200,1600)/dt))
...
def _delaydm(config, mode, stim_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim2_ons  = stim1_offs + int(rng.uniform(400,1400)/dt)
...
def dmc_(config, mode, matchnogo, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim1_offs = stim1_ons + int(rng.uniform(200,600)/dt)
        stim2_ons  = stim1_offs + int(rng.uniform(200,1600)/dt)
        tdim       = stim2_ons + int(rng.uniform(200,600)/dt)
